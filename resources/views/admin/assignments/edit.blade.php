...
<head>
    ...
    <title>Edit Assignment - Course Management System</title>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="d-flex align-items-center"><i class="fas fa-edit me-2" style="font-size: 1.5rem;"></i>Edit Assignment</h2>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ auth()->user()->role === 'lecturer' ? route('lecturer.assignments.update', $assignment) : route('admin.assignments.update', $assignment) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                          ...
                                          placeholder="Enter assignment description, instructions, and requirements" required>{{ old('description', $assignment->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-4">
                                <div class="card bg-light">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <strong>Course:</strong> <span id="preview-course">{{ $assignment->course->code }} - {{ $assignment->course->title }}</span><br>
                                                <strong>Title:</strong> <span id="preview-title">{{ $assignment->title }}</span><br>
                                                <strong>Due Date:</strong> <span id="preview-due-date">{{ $assignment->due_date->format('M d, Y h:i A') }}</span>
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Max Score:</strong> <span id="preview-max-score">{{ $assignment->max_score }}</span><br>
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <div id="preview-description" class="mt-2 p-2 border rounded bg-white" style="white-space: pre-wrap;">{{ $assignment->description }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePreview() {
                ...
                if (selectedCourse.value) {
                    document.getElementById('preview-course').textContent = selectedCourse.textContent;
                }
                ...
                // Update max score
                document.getElementById('preview-max-score').textContent = maxScoreInput.value;
            }
        });
    </script>
</body>
